...
class RegionalMembershipPage extends StatefulWidget {

  const RegionalMembershipPage({super.key});

  @override
  State<RegionalMembershipPage> createState() => _RegionalMembershipPageState();
}

class _RegionalMembershipPageState extends State<RegionalMembershipPage> {
  late final bloc = context.read<HomeBloc>();

  @override
  void initState() {
    super.initState();
    bloc.regionalMembership();
  }
    ...
    return BlocBuilder<AppCubit, AppState>(
      builder: (context, appState) {
        return Scaffold(
          appBar: AppBar(
            title: Text(
              S.of(context).regional_membership,
            ),
          ),
                ...
                child: state.isLoading
                    ...
                    : MediaQuery.removePadding(
                        ...
                        child: ListView.builder(
                            ...
                            itemCount: state.regionalMembership?.length,
                            itemBuilder: (context, index) {
                              return buildLeadershipItem(
                                  context, state.regionalMembership?[index]);
                            }),
                      ),
              );
            },
